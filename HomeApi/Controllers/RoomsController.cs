...
namespace HomeApi.Controllers
{
    ...
    public class RoomsController : ControllerBase
    {
            ...
            [FromBody] EditRoomRequest request)
        {
            ...
            if (room == null)
            {
                return StatusCode(400, $"Ошибка: Комната c идентификатором: {id} не существует.");
            }
            await _repository.UpdateRoom(
                room, new UpdateRoomQuery(
                    request.NewName,
                    request.NewArea,
                    request.NewGasConnected,
                    request.NewVoltage));

            return StatusCode(200, $"Комната обновлена! Name - {room.Name}, Area - {room.Area}," +
                $"  GasConnected - {room.GasConnected}, Voltage - {room.Voltage}");
        }
    }
}
